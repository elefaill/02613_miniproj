...
def jacobi_all_in_one(u, interior_mask, max_iter, atol=1e-6):

    u = cp.copy(u)

    for i in range(max_iter):
        # Compute new values for all interior cells across all buildings
        u_new = 0.25 * (
        ...
        # Compute maximum update (residual) over all buildings
        delta = cp.abs(u_old[interior_mask] - u_new[interior_mask]).max()

        # Update interior values
        u_old[interior_mask] = u_new[interior_mask]
...
if __name__ == '__main__':
    ...
    all_u = jacobi_all_in_one(all_u0, all_interior_mask, MAX_ITER, ABS_TOL)
